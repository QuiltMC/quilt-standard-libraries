	...
	public void onInitializeClient(ModContainer mod) {
		...
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, buildContext, environment) -> {
			dispatcher.register(literal("client_tag_test")
					.then(literal("block")
							.executes(context -> {
								TagsTestMod.displayTag(TEST_CLIENT_BLOCK_TAG, BuiltinRegistries.BLOCK, FEEDBACK_CONSUMER);
								return 0;
							})
					).then(literal("biome")
							.executes(context -> {
							...
							.executes(context -> {
								TagsTestMod.displayTag(TEST_DEFAULT_ITEM_TAG, BuiltinRegistries.ITEM, FEEDBACK_CONSUMER);
								return 0;
							})
					).then(literal("derivative_block")
							.executes(context -> {
								TagsTestMod.displayTag(TEST_CLIENT_DERIVATIVE_BLOCK_TAG, BuiltinRegistries.BLOCK, FEEDBACK_CONSUMER);
								return 0;
							})
					)
			);
