	...
	public Identifier getQuiltId() {
		if (this.quilt$id == null) {
			...
			if (self instanceof RecipeManager) {
				this.quilt$id = ResourceReloaderKeys.RECIPES;
			} else if (self instanceof ServerAdvancementLoader) {
				this.quilt$id = ResourceReloaderKeys.ADVANCEMENTS;
			} else if (self instanceof FunctionLoader) {
				this.quilt$id = ResourceReloaderKeys.FUNCTIONS;
			} else if (self instanceof LootManager) {
				this.quilt$id = ResourceReloaderKeys.LOOT_TABLES;
			} else if (self instanceof TagManagerLoader) {
				this.quilt$id = ResourceReloaderKeys.TAGS;
			} else {
				this.quilt$id = new Identifier("minecraft",
	...
	public Collection<Identifier> getQuiltDependencies() {
		if (this.quilt$dependencies == null) {
			...
			if (self instanceof TagManagerLoader) {
				...
				this.quilt$dependencies = Collections.singletonList(ResourceReloaderKeys.TAGS);
			}
		}
