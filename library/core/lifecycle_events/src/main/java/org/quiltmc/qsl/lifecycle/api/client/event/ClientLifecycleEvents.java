...
public final class ClientLifecycleEvents {
	// There is no STARTING event because there is no way to allow mods to register callbacks that early without
	 ...
	 *
	 * <p>After this event is fired, the client will disconnect from the server if it is connected to one. Then, if the
	 * client was running an integrated server, the integrated server will be shut down. Finally, all client facilities
	 * are torn down.
	 *
	 * <h2>What should mods do when this event is executed?</h2>
	 *
	 * <p>Mods which maintain session data when connected to a server should save that data here, as the client will
	 * still have access to the connected server.
	 *
