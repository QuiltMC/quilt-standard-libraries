 ...
 *
 * <p>The Minecraft login protocol only allows the client to respond to a server's request,
 * but not initiate one of its own.
 *
...
public final class ClientLoginNetworking {
	 ...
	 * Registers a handler to a query request channel.
	 *
	 * <p>If a handler is already registered to the {@code channelName}, this method will return {@code false},
	 * and no change will be made.
	...
	public interface QueryRequestReceiver {
		 ...
		 *
		 * <p>The return value of this method is a completable future that may be used to delay the login process to
		 * the server until a task {@link CompletableFuture#isDone() is done}.
		 * The future should complete in reasonably time to prevent disconnection by the server.
		 * If your request processes instantly, you may use {@link CompletableFuture#completedFuture(Object)}
		 ...
		 * If the future contains {@code null}, then the server will be notified that the client did not understand the
		 * query.
		 */
